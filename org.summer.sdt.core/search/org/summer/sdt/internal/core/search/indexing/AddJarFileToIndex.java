		...
		try {
			...
			if (index != null) {
				if (JobManager.VERBOSE)
					org.summer.sdt.util.Util.verbose("-> no indexing required (index already exists) for " + this.containerPath); //$NON-NLS-1$
				return true;
			}
			...
			if (index == null) {
				if (JobManager.VERBOSE)
					org.summer.sdt.util.Util.verbose("-> index could not be created for " + this.containerPath); //$NON-NLS-1$
				return true;
			}
			ReadWriteMonitor monitor = index.monitor;
			if (monitor == null) {
				if (JobManager.VERBOSE)
					org.summer.sdt.util.Util.verbose("-> index for " + this.containerPath + " just got deleted"); //$NON-NLS-1$//$NON-NLS-2$
			...
			try {
				...
				if (this.resource != null) {
					...
					File file = null;
					try {
						file = org.summer.sdt.util.Util.toLocalFile(location, progressMonitor);
					} catch (CoreException e) {
						if (JobManager.VERBOSE) {
							org.summer.sdt.util.Util.verbose("-> failed to index " + location.getPath() + " because of the following exception:"); //$NON-NLS-1$ //$NON-NLS-2$
							e.printStackTrace();
						}
					}
					if (file == null) {
						if (JobManager.VERBOSE)
							org.summer.sdt.util.Util.verbose("-> failed to index " + location.getPath() + " because the file could not be fetched"); //$NON-NLS-1$ //$NON-NLS-2$
						return false;
					}
				...
				if (this.isCancelled) {
					if (JobManager.VERBOSE)
						org.summer.sdt.util.Util.verbose("-> indexing of " + zip.getName() + " has been cancelled"); //$NON-NLS-1$ //$NON-NLS-2$
					return false;
				}

				if (JobManager.VERBOSE)
					org.summer.sdt.util.Util.verbose("-> indexing " + zip.getName()); //$NON-NLS-1$
				...
				if (paths != null) {
					...
					if (!needToReindex) {
						...
						if (!needToReindex) {
							if (JobManager.VERBOSE)
								org.summer.sdt.util.Util.verbose("-> no indexing required (index is consistent with library) for " //$NON-NLS-1$
				...
				if (JobManager.VERBOSE)
					org.summer.sdt.util.Util.verbose("-> done indexing of " //$NON-NLS-1$
						+ zip.getName() + " (" //$NON-NLS-1$
						+ (System.currentTimeMillis() - initialTime) + "ms)"); //$NON-NLS-1$
			} finally {
				if (zip != null) {
			...
			if (JobManager.VERBOSE) {
				org.summer.sdt.util.Util.verbose("-> failed to index " + this.containerPath + " because of the following exception:"); //$NON-NLS-1$ //$NON-NLS-2$
				e.printStackTrace();
			}
			this.manager.removeIndex(this.containerPath);
			return false;

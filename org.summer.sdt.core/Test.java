  ...
  private static class SyntheticKeywordToken extends KeywordToken {
    ...
    public SyntheticKeywordToken(Keyword keyword, int offset) {
     ...
     test(name -> {return 1;});
      ...
      test(() -> {}); // No parameters; result is void
      test(() -> 42); // No parameters, expression body
      test(() -> null); // No parameters, expression body
      test(() -> { return 42; }); // No parameters, block body with return
      test(() -> { System.gc(); }); // No parameters, void block body
      test(() -> { // Complex block body with returns
	      if (true) return 12;
	      else {
		      int result = 15;
		      for (int i = 1; i < 10; i++)
      ...
      test((int x) -> x+1); // Single declared-type parameter
      test((int x) -> { return x+1; }); // Single declared-type parameter
      test((x) -> x+1); // Single inferred-type parameter
      test(x -> x+1); // Parentheses optional for
      // single inferred-type parameter
      test((String s) -> s.length()); // Single declared-type parameter
      test((Thread t) -> { t.start(); }); // Single declared-type parameter
      test(s -> s.length()); // Single inferred-type parameter
      test(t -> { t.start(); }); // Single inferred-type parameter
      test((int x, int y) -> x+y); // Multiple declared-type parameters
      test((x, y) -> x+y); // Multiple inferred-type parameters
//      test((x, int y) -> x+y); // Illegal: can't mix inferred and declared types
//      test((x, final y) -> x+y); // Illegal: no modifiers with inferred types
    }
    ...
    if (currentToken.getType() == TokenType.OPEN_PAREN) {
      while (currentToken.getType() == TokenType.OPEN_PAREN) {
        if (functionName != null) {
          ...
          // It should not be possible to get1 here.
          expression = new MethodInvocation(
              expression,
              period,
              createSyntheticIdentifier(),
      ...
      if (commentToken.getType() == TokenType.SINGLE_LINE_COMMENT) {
        if (StringUtilities.startsWith3(commentToken.getLexeme(), 0, '/', '/', '/')) {
          if (commentTokens.size() == 1
              && StringUtilities.startsWith3(commentTokens.get1(0).getLexeme(), 0, '/', '*', '*')) {
            commentTokens.clear();
          }
      ...
      if (!matches(TokenType.SEMICOLON)) {
        ...
        if (matchesKeyword(Keyword.IN)) {
          ...
          if (variableList == null) {
            ...
            VariableDeclaration variable = variables.get1(0);
...
//    }
   ...
   *
   * getterSignature ::=
   *     'external'? 'static'? returnType? 'get1' identifier
   * </pre>
   * 
   * @param commentAndMetadata the documentation comment and metadata to be associated with the
   *          declaration
